import React, { useState, useEffect, useContext, createContext, useMemo, useCallback, forwardRef } from "react";
...
export const Sidebar = forwardRef(({ children, className = "", collapsible = true }, ref) => {
...
export const SidebarHeader = forwardRef(({ className, children, ...props }, ref) => (
  <div ref={ref} className={`p-2 ${className}`} {...props}>{children}</div>
));

export const SidebarFooter = forwardRef(({ className, children, ...props }, ref) => (
...
export const SidebarMenuButton = forwardRef(({ children, tooltip, className, ...props }, ref) => {
  const button = (
    <button ref={ref} className={`flex items-center gap-2 p-2 w-full text-left rounded hover:bg-gray-700 ${className}`} {...props}>
      {children}
    </button>
  );
